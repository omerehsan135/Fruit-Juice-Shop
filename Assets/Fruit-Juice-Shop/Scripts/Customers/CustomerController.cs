...
public class CustomerController : MonoBehaviour {
	...
	*/
	...
	void Init (){
		...
		if(customerNeeds == 0){
			...
			if(PlayerPrefs.GetString("gameMode") == "CAREER") {
				...
				customerNeeds = UnityEngine.Random.Range(0, availableProducts.Length);
			}
		}	
	...
	IEnumerator goToSeat (){
		timeVariance = UnityEngine.Random.value;
		while(!isOnSeat) {
	...
	void LateUpdate (){
		...
		if(target) {
			if(target.GetComponent<BlenderController>().isReadyToServe && gameObject.tag == "customer")
